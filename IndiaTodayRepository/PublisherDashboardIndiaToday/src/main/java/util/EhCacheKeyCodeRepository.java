...
public class EhCacheKeyCodeRepository implements KeyCodeRepository {
	...
	public CSVResult get(String key) {
		...
		if (key.contains(date) && !key.contains("http") && !key.contains("refcurrentoriginal") && !key.contains("COUNT(*)as count,SUM(engagementTime),cookie_id") && !key.contains("count(*),count(DISTINCT(sessionhash)),cookiehash") && !key.contains("count(*),max(request_time),sessionhash") && !key.contains("count(*),max(request_time),min(request_time),sessionhash") && !key.contains("count(*),count(DISTINCT(refcurrentoriginal)),sessionhash")) {
