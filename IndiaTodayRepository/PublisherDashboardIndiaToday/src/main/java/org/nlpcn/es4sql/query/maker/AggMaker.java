...
//    private static final String TIME_FARMAT = "HH:mm:ss";
	...
	private AbstractAggregationBuilder makeCountAgg(MethodField field) {
		...
		if ("DISTINCT".equals(field.getOption())) {
			...
			if (field.getAlias().contains("refcurrentoriginal")) {
				return AggregationBuilders.cardinality(field.getAlias()).precisionThreshold(1).field(field.getParams().get(0).value.toString());
			}
			
			return AggregationBuilders.cardinality(field.getAlias()).precisionThreshold(1).rehash(false).field(field.getParams().get(0).value.toString());
		}
