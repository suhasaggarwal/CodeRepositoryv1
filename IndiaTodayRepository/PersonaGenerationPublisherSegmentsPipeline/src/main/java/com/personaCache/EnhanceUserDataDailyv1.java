...
static Integer recordCount = 0;
	...
	//	Client client = new TransportClient()
		...
		try {
			System.setOut(new PrintStream(new File("PersonaSyncLogv1.txt")));
		} catch (Exception e) {
			e.printStackTrace();
		}
	...
	//	 EhCacheKeyCodeRepository ehcache = EhCacheKeyCodeRepository.getInstance();
		...
		/*
		 Set<String> cookieList = new LinkedHashSet<String>();
		 for (String cookie : cookieListDuplicate) {
		...
		try{
			...
			//while (true) {
				...
				for (String cookie : cookieList) {
					...
					lst.add(new WorkerThread3(client,cookie, referrerType, affinitySegments, sections, tags, writer));
				}
