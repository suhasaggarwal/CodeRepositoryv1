...
personaSignalsbatch = []
colormap = {}
ehcacheDatabase = open('EhcacheUploadFilepart2.txt', "w")
...
if __name__ == '__main__':
    ...
    Persona_Data_File = path + "/OPD12.txt"
    ...
    print('Persona Segment Affinity Computation Worker 2')
    database = ""
    ...
    for epersona in enhancedPersona:
        if epersona is not None:
            ...
            if personacolor is not None:
                ...
                if not matchingcolorkey and not cookie_id_color:
                   userdoccolorseg[cookie_id_color] = colormap[matchingcolorkey.strip()].rstrip(',\n').strip()
                #print(userdoccolorseg[cookie_id_color])
                if personacolor is not None:
    ...
    for key in userdocuserid.keys():
        ...
        try:
            ...
            mlsignals = userdocsectionseg[key].rstrip(",") + "," + userdocemotionseg[key].rstrip(",") + "," + \
                        userdoccolorseg[key].rstrip(",") + "," + userdocfpsa[key].rstrip(",") + "," + userdocfpca[
                            key].rstrip(",")
            #print(mlsignals)
            ...
            #print(ehcacheData)
            ehcacheData = ehcacheData.replace("1000", "0").replace(",,", ",")
            ehcacheDatabase.write(ehcacheData)
        except Exception:
            traceback.print_exc()
            continue
