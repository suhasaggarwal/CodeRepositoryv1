...
def sif_embeddings(sentence, embeddingType, wordset ,alpha=1e-3):
    """Compute the SIF embedings for a list of sentences
    Parameters
    ----------
    sentences : list
        ...
        if entityType == "Persona":
            for entity in entityList:
                try:
                    ...
                    twitter_avg_vector = sif_embeddings(personaData.split(','), "twitter", tindex2word_set,
                                                            num_features=400)
                    ...
                    wiki_avg_vector = sif_embeddings(personaData.split(','), "wiki", windex2word_set,
                                                         num_features=300)
                    twitter_avg_vector = sif_embeddings(personaData.split(','), "twitter", tindex2word_set,
                                                            num_features=400)
                    conceptnet_avg_vector = sif_embeddings(personaData.split(','), "conceptnet", cindex2word_set,
                                                               num_features=300)
            ...
            try:
                for entity in entityList:
                    ...
                    twitter_avg_vector = sif_embeddings(cpersonaData.split(','), "twitter", tindex2word_set,
                                                            num_features=400)
                    emotion = computeEmotionsAffinity(emotionDatabaseBuffer, entityType, cpersonaData.split(','),
                                                      creativeId, twitter_avg_vector, "twitter")
                    # entity = entity.rstrip("\n") + "," + emotion
                    wiki_avg_vector = sif_embeddings(cpersonaData.split(','), "wiki", windex2word_set,
                                                         num_features=300)
                    twitter_avg_vector = sif_embeddings(cpersonaData.split(','), "twitter", tindex2word_set,
                                                            num_features=400)
                    conceptnet_avg_vector = sif_embeddings(cpersonaData.split(','), "conceptnet", cindex2word_set,
                                                               num_features=300)
...
def avg_feature_vec(words, embeddingType, index2word_set, num_features):
        ...
        try:
            ...
            if embeddingType == "wiki":
                sen1_sen2_similarity = cosine_similarity(sentence_1_avg_vector,
                                                         sif_embeddings(y.split(), "wiki", windex2word_set,
                                                                            num_features=300))
            if embeddingType == "twitter":
                sen1_sen2_similarity = cosine_similarity(sentence_1_avg_vector,
                                                         sif_embeddings(y.split(), "twitter", tindex2word_set,
                                                                            num_features=400))
            if embeddingType == "conceptnet":
                sen1_sen2_similarity = cosine_similarity(sentence_1_avg_vector,
                                                         sif_embeddings(y.split(), "conceptnet", cindex2word_set,
                                                                            num_features=300))
...
def generateSegmentsVectorFile(model, dimensions, embeddingtype, filename):
    ...
    for y in content1:
        ...
        if value is not None:
            ...
            segment_vector = sif_embeddings(sentence_2.split(), model, num_features=dimensions)
        ...
        for y in content1:
            try:
                ...
                if value is not None:
                    ...
                    t00 = time.time()
                    topic_vector = sif_embeddings(sentence_2.replace("\r", "").replace("\n", " ").lower().split(),
        ...
        for y in content1:
            try:
                ...
                if value is not None:
                    ...
                    t00 = time.time()
                    segment_vector_wiki = sif_embeddings(
                        sentence_2.replace("\r", "").replace("\n", " ").lower().split(),
                        "wiki", windex2word_set, num_features=300)
                    segment_vector_twitter = sif_embeddings(
                        sentence_2.replace("\r", "").replace("\n", " ").lower().split(),
                        "twitter", tindex2word_set, num_features=400)
                    segment_vector_concept = sif_embeddings(
                        sentence_2.replace("\r", "").replace("\n", " ").lower().split(),
...
class SegmentSegmentAffinityVectorComputationWorker:
    ...
    def run(self, dimensions, segWiki, segmentList, segsegAffinityWiki):
        ...
        content1 = [x.strip() for x in segmentList]
        for y in content1:
            segment_vector = sif_embeddings(y.replace("\r", "").replace("\n", " ").lower().split(),
                                                "wiki", windex2word_set, num_features=dimensions)
            for segKey, segValue in segWiki.__iter__():
                try:
...
personafname = 'split_file_datav00.txt'
