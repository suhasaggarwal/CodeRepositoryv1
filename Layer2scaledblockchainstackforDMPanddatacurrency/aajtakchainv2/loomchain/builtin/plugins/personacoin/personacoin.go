...
//Awarded on performing following actions (Amount of persona coins can be awarded according to weight) -
//1)In case of comment - social network activity
//2)In case of news digest subscription points
//3)login points
//4)In case of comments reaches a certain threshold
//5)Newsletter subscription
//6)Article views
//7)Engagement Time
//8)User 360 profile
//9)New,Loyal,Returning users - in each segment (according to taxonomy)
//10)Geographic Persona
//11)Device Persona
//12)Emotion Persona
//13)Income Level Persona
//14)Aajtak data smart contract will have initial minted supply of persona coins.
//In case of activity in above domain, persona weight in different domains/segments exceed a certain threshold, in that case certain amount of persona coins will be transferred from website to the user
//All the transactions can be viewed in grafana, events, user balances
...
func (c *PersonaCoin) Init(ctx contract.Context, req *InitRequest) error {
...
func (c *PersonaCoin) MintToGateway(ctx contract.Context, req *MintToGatewayRequest) error {
	if ctx.FeatureEnabled(loomchain.PersonaCoinVersion1_2Feature, false) && req.Amount == nil {
...
func (c *PersonaCoin) Burn(ctx contract.Context, req *BurnRequest) error {
...
func (c *PersonaCoin) TotalSupply(
...
func (c *PersonaCoin) Transfer(ctx contract.Context, req *TransferRequest) error {
	if ctx.FeatureEnabled(loomchain.PersonaCoinVersion1_2Feature, false) && (req.To == nil || req.Amount == nil) {
		return ErrInvalidRequest
	}
	if ctx.FeatureEnabled(loomchain.PersonaCoinVersion1_1Feature, false) {
...
func (c *PersonaCoin) Approve(ctx contract.Context, req *ApproveRequest) error {
	if ctx.FeatureEnabled(loomchain.PersonaCoinVersion1_2Feature, false) && (req.Spender == nil || req.Amount == nil) {
		return ErrInvalidRequest
	}
...
func (c *PersonaCoin) TransferFrom(ctx contract.Context, req *TransferFromRequest) error {
	if ctx.FeatureEnabled(loomchain.PersonaCoinVersion1_2Feature, false) {
...
var Contract plugin.Contract = contract.MakePluginContract(&PersonaCoin{})

// Legacy methods from v1.0.0

func (c *PersonaCoin) legacyTransfer(ctx contract.Context, req *TransferRequest) error {
	from := ctx.Message().Sender
	to := loom.UnmarshalAddressPB(req.To)
...
func (c *PersonaCoin) legacyTransferFrom(ctx contract.Context, req *TransferFromRequest) error {
	from := loom.UnmarshalAddressPB(req.From)
	fromAccount, err := loadAccount(ctx, from)
	if err != nil {
		return err
