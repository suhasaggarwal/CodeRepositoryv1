...
//Awarded on performing following actions (Amount of aajtak coins can be awarded according to aajtak weight) -
...
//6)Article views
//7)Engagement Time (Article page, videos)
//8)User 360 profile
//9)CLi which will be there on top of Aajtak coin smart contract - 
//10)Two types of addresses - 
//1)website (Minted amount)
//2)user (For user activity on the website in above domain, aajtak coins will be transferred from website to the user)
...
type AajtakCoin struct {
}

func (c *AajtakCoin) Meta() (plugin.Meta, error) {
	return plugin.Meta{
		Name:    "coin",
		Version: "1.0.0",
	}, nil
...
func (c *AajtakCoin) Init(ctx contract.Context, req *InitRequest) error {
...
func (c *AajtakCoin) MintToGateway(ctx contract.Context, req *MintToGatewayRequest) error {
...
func (c *AajtakCoin) Burn(ctx contract.Context, req *BurnRequest) error {
...
func (c *AajtakCoin) Transfer(ctx contract.Context, req *TransferRequest) error {
	if ctx.FeatureEnabled(loomchain.AajtakCoinVersion1_1Feature, false) {
...
func (c *AajtakCoin) Approve(ctx contract.Context, req *ApproveRequest) error {
...
func (c *AajtakCoin) TransferFrom(ctx contract.Context, req *TransferFromRequest) error {
...
func (c *AajtakCoin) legacyTransfer(ctx contract.Context, req *TransferRequest) error {
	from := ctx.Message().Sender
	to := loom.UnmarshalAddressPB(req.To)
...
func (c *AajtakCoin) legacyTransferFrom(ctx contract.Context, req *TransferFromRequest) error {
	spender := ctx.Message().Sender
	from := loom.UnmarshalAddressPB(req.From)
	to := loom.UnmarshalAddressPB(req.To)
