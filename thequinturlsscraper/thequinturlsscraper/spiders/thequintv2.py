...
class QuintSpider(scrapy.Spider):
    name = 'quintv3'
    with open('/root/QuintUrlsFinalv2.txt') as f:
...
#        pattern = re.compile("author-name.*?[,]{1}")
       ...
       # author_text = soup.findall(re.compile("author-name.*?[,]{1}"))
        ...
        print(t)
        r1 = requests.get("http://192.168.103.138:5001/getTextAnalytics?url=" + response.url)
        r = requests.get("http://localhost:5000/IAB/" + path.lower().replace("-", " ").replace(","," ").replace(";"," "))
        ...
        d["Entity6"] = k
        d["Entity4"] = str(author.encode('ascii','ignore')).replace("b","").replace("'","")
        d["Entity7"] = c
        d["Entity5"] = n
        d["Entity3"] = r
        d["Entity8"] = ""
        ...
        import codecs
        fp = codecs.open('quintitemslatestfinalv2222.json', mode='a', encoding='utf-8')
        line1 = '{"index":{}}' + "\n"
